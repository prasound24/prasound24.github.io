...
export function interpolate_1d(a, b) {
  ...
  dcheck(b.length % 2 == 0);
  let aa = a.slice();
  bluestein_fft_1d(a, aa);
  bluestein_fft_1d(aa, b, -1);
}

// https://en.wikipedia.org/wiki/Analytic_signal
export function harmonic_conjugate(re, im = new Float32Array(re.length)) {
  let n = re.length;
  dcheck(im.length == n);
  ...
  mul_const(aa.subarray(2, n), 2);
  mul_const(aa.subarray(n + 1), 0);
