...
export const DEBUG = typeof location == 'undefined' || location.hostname == '0.0.0.0' || location.hostname == 'localhost';
...
export function rect2disk(rect, disk, { num_reps = 1, onprogress } = {}) {
  ...
  draw((x, y) => [x, y]);
  onprogress?.call(null, 0.25);
  draw((x, y) => [x, n - y - 1]);
  onprogress?.call(null, 0.50);
  draw((x, y) => [y, n - x - 1]);
  onprogress?.call(null, 0.75);
  draw((x, y) => [n - y - 1, x]);
  onprogress?.call(null, 1.00);
...
export function meanPitch(freq_hz) {
  ...
  return freq_hz > 0 ? ((Math.log2(freq_hz / c8_hz) % 1) + 1) % 1 : -1;
}
