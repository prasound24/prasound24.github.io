...
test('p-norm vs max', () => {
  for (let p of [2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]) {
    let sum = 0, max = 0, n = 100;
    for (let i = 0; i < n; i++) {
      let x = Math.random();
      sum += x ** p;
      max = Math.max(max, x);
    }
    console.log('|' + p + '-norm/max - 1|/n=' + (Math.abs(sum ** (1 / p) / max - 1) / n).toExponential(2));
  }
});
...
test('harmonic_conjugate', () => {
  ...
  webfft.harmonic_conjugate(a, b);
  webfft.harmonic_conjugate(b, c);
  console.log('a=', a);
  console.log('H(a)=', b);
  console.log('H(H(a))=', c);
});
